...
(async () => {
  ...
  console.log('   ...starting share server on 18099...');
  const shareProc = spawn(NODE_BIN, [CLI, 'share', '.', '--port', '18099', '--host', '127.0.0.1', '--expires', '5', '--type', 'file'], {
    cwd: SENDER_DIR,
  ...
  if (shareId) {
    const fetchRes = await runCli(['fetch', `http://127.0.0.1:18099/share/${shareId}`, '--output', RECEIVER_DIR], RECEIVER_DIR, 30000);
    ...
    // Folder shares arrive as <foldername>.zip — accept any non-empty zip in receiver dir
